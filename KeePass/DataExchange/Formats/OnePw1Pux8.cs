...
namespace KeePass.DataExchange.Formats
{
    ...
    internal sealed class OnePw1Pux8 : FileFormatProvider
    {
        ...
        {
            using (var za = new ZipArchive(sInput))
            {
                string str;
                using (var s = za.GetEntry("export.data").Open())
                {
        ...
        private static void ImportRoot(JsonObject jo, ZipArchive za, PwDatabase pd)
        {
        ...
        private static void ImportItem(JsonObject jo, ZipArchive za, PwGroup pg,
            PwDatabase pd)
        {
        ...
        private static void ImportSectionField(JsonObject jo, ZipArchive za, string strSection, PwEntry pe, PwDatabase pd)
        {
        ...
        private static void ImportAttachment(JsonObject jo, ZipArchive za, PwEntry pe)
        {
            ...
            byte[] pbData;
            using (Stream s = za.GetEntry($"files/{strDocId}__{strFileName}").Open())
            {
                if (s == null) throw new FormatException();
