...
namespace KeePass.Util
{
    ...
    public sealed class UpdateComponentInfo
    {
        ...
        public ulong VerInstalled { get; }
        public ulong VerAvailable { get; set; } = 0;
        public UpdateComponentStatus Status { get; set; } = UpdateComponentStatus.Unknown;
        public string UpdateUrl { get; }
        public string Category { get; }

        public UpdateComponentInfo(string strName, ulong uVerInstalled, string strUpdateUrl, string strCategory)
        {
            Name = strName ?? throw new ArgumentNullException("strName");
            VerInstalled = uVerInstalled;
            UpdateUrl = strUpdateUrl ?? throw new ArgumentNullException("strUpdateUrl");
            Category = strCategory ?? throw new ArgumentNullException("strCategory");
        }
    }

    public static class UpdateCheckEx
    {
        private static readonly Dictionary<string, string> g_dFileSigKeys = new Dictionary<string, string>();
        ...
        public static void Run(bool bForceUI, Form fOptParent)
        {
            ...
            var strLast = Program.Config.Application.LastUpdateCheck;

            if (!bForceUI && (strLast.Length > 0) && TimeUtil.TryDeserializeUtc(strLast, out var dtLast) && CompareDates(dtLast, dtNow) == 0)
                return; // Checked today already

            Program.Config.Application.LastUpdateCheck = TimeUtil.SerializeUtc(dtNow);

            var p = new UpdateCheckParams(bForceUI, fOptParent);

            if (bForceUI)
                RunPriv(p);
            else
                ThreadPool.QueueUserWorkItem(new WaitCallback(RunPriv), p);
        }
